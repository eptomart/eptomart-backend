...
const CATEGORIES = [
  // ── 8 Homepage Featured (shared / Eptomart-core) ──────────
  { name: 'Fruits',             icon: '🍊', moduleType: 'shared',          isFeatured: true,  sortOrder: 1  },
  { name: 'Vegetables',         icon: '🥦', moduleType: 'shared',          isFeatured: true,  sortOrder: 2  },
  { name: 'Flowers & Greens',   icon: '🌸', moduleType: 'shared',          isFeatured: true,  sortOrder: 3  },
  ...
  { name: 'Farm Fresh',         icon: '🌾', moduleType: 'shared',          isFeatured: true,  sortOrder: 6  },
  { name: 'Homemade & Organic', icon: '🏡', moduleType: 'eptomart',        isFeatured: true,  sortOrder: 7  },
  { name: 'Pooja & Coconut',    icon: '🪔', moduleType: 'shared',          isFeatured: true,  sortOrder: 8  },

  // ── Additional Eptomart categories ───────────────────────
...
async function seed() {
  try {
    ...
    for (const cat of CATEGORIES) {
      ...
      if (exists) {
        // Update moduleType + isFeatured if missing
        let updated = false;
        if (!exists.moduleType || exists.moduleType === 'eptomart') {
          exists.moduleType = cat.moduleType;
          updated = true;
        }
        if (exists.isFeatured === undefined) {
